...
import { GQL_USER_Two_Tier } from "~~/helpers/getQueries";

export const UsersTable = () => {
  const [pageSize, setPageSize] = useState(25);
  const [pageNum, setPageNum] = useState(0);

  const { loading, error, data } = useQuery(GQL_USER_Two_Tier(), {
    variables: {
      limit: pageSize,
  ...
  useEffect(() => {
    if (error !== undefined && error !== null) console.log("UsersTable.tsx Query Error: ", error);
  }, [error]);

  useEffect(() => {
    if (data !== undefined && data !== null) console.log("UsersTable.tsx Query data: ", data);
  }, [data]);
    ...
    return (
      <>
        <div className="flex gap-3 mb-3">
          <span className="my-auto text-lg">LATEST USERS</span>
          <select
